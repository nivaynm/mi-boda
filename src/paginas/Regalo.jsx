...
function Regalo({ t }) {
  ...
  const datosBanco = t.idioma === "en" ? [
    { label:"Account holder", valor:"Gabriel Illan" },
    { label:"Bank", valor:"Revolut Ltd" },
    { label:"Country", valor:"United Kingdom" },
    { label:"Address", valor:"Porteus Road, London" },
  ] : [
  ...
  return (
    <div style={{ minHeight:"100vh", background:"#fdf6f0", fontFamily:"Georgia, serif", padding:"50px 20px 100px" }}>
      ...
      <div style={{ textAlign:"center", marginBottom:"36px" }}>
        <p style={{ color:"#b89a7a", letterSpacing:"4px", fontSize:"12px", margin:"0 0 8px" }}>{t.vuestraAportacion}</p>
        <h2 style={{ color:"#4a3728", fontSize:"28px", fontWeight:"normal", margin:0 }}>🎁 {t.idioma === "en" ? "Gift" : "Regalo"}</h2>
      </div>
      ...
      <div style={{ maxWidth:"480px", margin:"0 auto" }}>
        ...
        <div style={{ background:"white", border:"1px solid #e8d5c4", borderRadius:"14px", padding:"24px", marginBottom:"16px", textAlign:"center" }}>
          ...
          <p style={{ fontSize:"15px", color:"#7a5c4a", lineHeight:"1.8", margin:"0 0 14px" }}>{t.mensajeRegalo}</p>
          <p style={{ fontSize:"17px", color:"#4a3728", margin:"16px 0 0", lineHeight:"1.6" }}>{t.lunaDeHiel}</p>
        </div>

        {/* De parte de */}
        <div style={{ background:"white", border:"1px solid #e8d5c4", borderRadius:"14px", padding:"24px", marginBottom:"16px" }}>
          <p style={{ color:"#b89a7a", letterSpacing:"3px", fontSize:"11px", margin:"0 0 14px" }}>{t.departe}</p>
          <p style={{ fontSize:"13px", color:"#7a5c4a", margin:"0 0 8px" }}>{t.departeTexto}</p>
          <input type="text" placeholder={t.idioma === "en" ? "Your name" : "Tu nombre"} value={nombre} onChange={(e) => setNombre(e.target.value)} style={{ ...inputStyle, marginBottom:"12px" }} />
          {enviado ? (
            <p style={{ color:"#b89a7a", fontSize:"13px", textAlign:"center", margin:0 }}>✓ {t.idioma === "en" ? `Thank you, ${nombre}! 🤍` : `¡Gracias, ${nombre}! 🤍`}</p>
          ) : (
            <button onClick={handleEnviar} style={{ width:"100%", background:"#4a3728", color:"white", border:"none", borderRadius:"20px", padding:"12px", fontSize:"14px", cursor:"pointer", fontFamily:"Georgia, serif" }}>
              {t.guardarNombre}
            </button>
          )}
        </div>
        ...
        <div style={{ background:"white", border:"1px solid #e8d5c4", borderRadius:"14px", padding:"24px", marginBottom:"16px" }}>
          ...
          <p style={{ color:"#b89a7a", letterSpacing:"3px", fontSize:"11px", textAlign:"center", margin:"0 0 20px" }}>{t.transferencia}</p>

          {datosBanco.map(({ label, valor }) => (
          ...
          <div style={{ background:"#fdf6f0", borderRadius:"10px", padding:"14px 16px", margin:"16px 0 12px" }}>
            ...
            <button onClick={() => copiar("GB13REVO00997052226873", setCopiadoIBAN)} style={{ background:"#4a3728", color:"white", border:"none", borderRadius:"20px", padding:"8px 20px", fontSize:"13px", cursor:"pointer" }}>
              {copiadoIBAN ? t.copiado : t.copiarIBAN}
            </button>
          </div>

          <div style={{ background:"#fdf6f0", borderRadius:"10px", padding:"14px 16px" }}>
            <p style={{ fontSize:"11px", color:"#b89a7a", letterSpacing:"2px", margin:"0 0 6px" }}>{t.concepto}</p>
            <p style={{ fontSize:"14px", color:"#4a3728", margin:"0 0 10px" }}>
              {t.idioma === "en" ? "Wedding of Gabriel and Vania" : "Boda de Gabriel y Vania"}
            </p>
            <button onClick={() => copiar(t.idioma === "en" ? "Wedding of Gabriel and Vania" : "Boda de Gabriel y Vania", setCopiadoRef)} style={{ background:"#4a3728", color:"white", border:"none", borderRadius:"20px", padding:"8px 20px", fontSize:"13px", cursor:"pointer" }}>
              {copiadoRef ? t.copiado : t.copiarConcepto}
            </button>
          </div>
        </div>
        ...
        <div style={{ background:"white", border:"1px solid #e8d5c4", borderRadius:"14px", padding:"24px", marginBottom:"16px" }}>
          ...
          <div style={{ background:"#fdf6f0", borderRadius:"10px", padding:"14px 16px", marginBottom:"12px" }}>
            <p style={{ fontSize:"11px", color:"#b89a7a", letterSpacing:"2px", margin:"0 0 6px" }}>{t.idioma === "en" ? "NUMBER" : "NÚMERO"}</p>
            ...
            <button onClick={() => copiar("603635721", setCopiadoBizum)} style={{ background:"#4a3728", color:"white", border:"none", borderRadius:"20px", padding:"8px 20px", fontSize:"13px", cursor:"pointer" }}>
              {copiadoBizum ? t.copiado : t.copiarNumero}
            </button>
          </div>
          <p style={{ fontSize:"12px", color:"#b89a7a", textAlign:"center", margin:0 }}>
            {t.idioma === "en" ? "Please include your name as the reference 🤍" : "Indica tu nombre en el concepto del Bizum 🤍"}
          </p>
        </div>

        <p style={{ fontSize:"12px", color:"#b89a7a", textAlign:"center", lineHeight:"1.8" }}>
          {t.idioma === "en" ? "Thank you so much for your generosity! 🌙" : "¡Muchas gracias por vuestra generosidad! 🌙"}
        </p>
